...
EnhancementsWidget::EnhancementsWidget(GraphicsWindow* parent)
{
  ...
  ConnectWidgets();
  AddDescriptions();
  connect(parent, &GraphicsWindow::BackendChanged, this, &EnhancementsWidget::LoadSettings);
}
...
void EnhancementsWidget::CreateWidgets()
{
  ...
  m_aa_combo = new ConfigComplexChoice<u32, bool>(Config::GFX_MSAA, Config::GFX_SSAA, m_game_layer);
  m_aa_combo->Add(tr("None"), 1, false);

  // Local game AA changes are normally disabled, because it's difficult to get the backend info to
  // populate the options. Only enable if global backend matches the local choice.
  m_aa_combo->setEnabled(m_game_layer == nullptr);

  m_texture_filtering_combo = new ConfigComplexChoice<int, TextureFilteringMode>(
      Config::GFX_ENHANCE_MAX_ANISOTROPY, Config::GFX_ENHANCE_FORCE_TEXTURE_FILTERING,
      m_game_layer);

  m_texture_filtering_combo->Add(tr("Default"), ANISO_DEFAULT, TextureFilteringMode::Default);
  m_texture_filtering_combo->Add(tr("2x Anisotropic"), ANISO_2X, TextureFilteringMode::Default);
  m_texture_filtering_combo->Add(tr("4x Anisotropic"), ANISO_4X, TextureFilteringMode::Default);
  m_texture_filtering_combo->Add(tr("8x Anisotropic"), ANISO_8X, TextureFilteringMode::Default);
  m_texture_filtering_combo->Add(tr("16x Anisotropic"), ANISO_16X, TextureFilteringMode::Default);
  m_texture_filtering_combo->Add(tr("Force Nearest"), ANISO_DEFAULT, TextureFilteringMode::Nearest);
  m_texture_filtering_combo->Add(tr("Force Linear"), ANISO_DEFAULT, TextureFilteringMode::Linear);
  m_texture_filtering_combo->Add(tr("Force Linear and 2x Anisotropic"), ANISO_2X,
                                 TextureFilteringMode::Linear);
  m_texture_filtering_combo->Add(tr("Force Linear and 4x Anisotropic"), ANISO_4X,
                                 TextureFilteringMode::Linear);
  m_texture_filtering_combo->Add(tr("Force Linear and 8x Anisotropic"), ANISO_8X,
                                 TextureFilteringMode::Linear);
...
void EnhancementsWidget::ConnectWidgets()
{
  ...
  connect(&Settings::Instance(), &Settings::ConfigChanged, this, &EnhancementsWidget::LoadSettings);
}
...
template <typename T>
void EnhancementsWidget::WriteSetting(const Config::Info<T>& setting, const T& value)
{
  if (m_game_layer != nullptr)
  {
    m_game_layer->Set(setting, value);
...
void EnhancementsWidget::LoadPPShaders(StereoMode stereo_mode)
{
  ...
  if (!found)
  {
    ...
    if (ReadSetting(Config::GFX_ENHANCE_POST_SHADER) != selected_shader)
      WriteSetting(Config::GFX_ENHANCE_POST_SHADER, selected_shader);
  }
...
void EnhancementsWidget::LoadSettings()
{
  ...
  LoadPPShaders(ReadSetting(Config::GFX_STEREO_MODE));
}

void EnhancementsWidget::UpdateAAModes()
{
  // Update possible AA options
  ...
  if (g_Config.backend_info.bSupportsSSAA || m_game_layer != nullptr)
  {
    for (const u32 aa_mode : aa_modes)
    {
